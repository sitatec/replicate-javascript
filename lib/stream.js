...
function createReadableStream({ url, fetch, options = {} }) {
  const { useFileOutput = true, maxWaitTimeInSec = 5 * 60, headers = {}, ...initOptions } = options;
  ...
  return new ReadableStream({
    async start(controller) {
      ...
      let done = false;
      const startTimeInSec = Date.now() / 1000
      ...
      while (true) {
        ...
        const timeTaken = (Date.now() / 1000) - startTimeInSec;
        const hasReachedMaxWaitTime = timeTaken >= maxWaitTimeInSec;
        if (done || hasReachedMaxWaitTime) {
          if (hasReachedMaxWaitTime && !done) {
            console.warn("SSE Server timed out and max wait time reached. Time taken: ", timeTaken);
          }
          break;
        } else {
          console.warn("SSE Server timed out but max wait time not reached, retrying. Time taken: ", timeTaken);
        }
      }
      controller.close();
    },
